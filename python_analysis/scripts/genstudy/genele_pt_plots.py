...
# Plot settings
plot_dict = {
    'variable': ['gen_leading_ele_pt', 'gen_subleading_ele_pt'],
    'year': 2024,
    'cut': 'cut1',
}
...
plt.title(rf'Gen Electron $p_T$ (pre-vtx sel): $M_1$ = {m1s}, $\Delta$ = {deltas}, c$\tau$ = {ctaus}mm')
plt.legend()
plt.savefig(f"{plotdir}/hist_prevtx_gen-ele-pt_delta-{utils.stringfy_friendly(deltas[0])}_ctau-{utils.stringfy_friendly(ctaus[0])}_m1-narrow.png")
...
plt.title(rf'Gen Electron $p_T$ (pre-vtx sel): $M_1$ = {m1s}, $\Delta$ = {deltas}, c$\tau$ = {ctaus}mm')
plt.legend()
plt.savefig(f"{plotdir}/hist_prevtx_gen-ele-pt_delta-{utils.stringfy_friendly(deltas[0])}_ctau-{utils.stringfy_friendly(ctaus[0])}_m1-wide.png")
...
plt.title(rf'Gen Electron $p_T$ (pre-vtx sel): $M_1$ = {m1s}, $\Delta$ = {deltas}, c$\tau$ = {ctaus}mm')
plt.legend()
plt.savefig(f"{plotdir}/hist_prevtx_gen-ele-pt_m1-{utils.stringfy_friendly(m1s[0])}_delta-{utils.stringfy_friendly(deltas[0])}.png")
...
plt.title(rf'Gen Electron $p_T$ (pre-vtx sel): $M_1$ = {m1s}, $\Delta$ = {deltas}, c$\tau$ = {ctaus}mm')
plt.legend()
plt.savefig(f"{plotdir}/hist_prevtx_gen-ele-pt_m1-{utils.stringfy_friendly(m1s[0])}_ctau-{utils.stringfy_friendly(ctaus[0])}.png")
