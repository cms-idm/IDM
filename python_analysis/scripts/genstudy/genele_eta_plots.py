...
# Plot settings
plot_label = 'gen-ele-eta'
sel_label = 'prevtx'
plot_title = r'Gen Electron $\eta$ (pre-vtx sel)'
plot_dict = {
    'variable': ['gen_leading_ele_eta', 'gen_subleading_ele_eta'],
    'year': 2024,
    'cut': 'cut5',
}
...
plt.legend()
plt.savefig(f"plots/hist_{sel_label}_{plot_label}_delta-{utils.stringfy_friendly(deltas[0])}_ctau-{utils.stringfy_friendly(ctaus[0])}_m1-narrow.png")
...
plt.title(rf'{plot_title}: $M_1$ = {m1s}, $\Delta$ = {deltas}, c$\tau$ = {ctaus}mm')
plt.legend()
plt.savefig(f"plots/hist_{sel_label}_{plot_label}_delta-{utils.stringfy_friendly(deltas[0])}_ctau-{utils.stringfy_friendly(ctaus[0])}_m1-wide.png")
...
plt.title(rf'{plot_title}: $M_1$ = {m1s}, $\Delta$ = {deltas}, c$\tau$ = {ctaus}mm')
plt.legend()
plt.savefig(f"plots/hist_{sel_label}_{plot_label}_m1-{utils.stringfy_friendly(m1s[0])}_delta-{utils.stringfy_friendly(deltas[0])}.png")
...
plt.title(rf'{plot_title}: $M_1$ = {m1s}, $\Delta$ = {deltas}, c$\tau$ = {ctaus}mm')
plt.legend()
plt.savefig(f"plots/hist_{sel_label}_{plot_label}_m1-{utils.stringfy_friendly(m1s[0])}_ctau-{utils.stringfy_friendly(ctaus[0])}.png")
