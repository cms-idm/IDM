...
class myHisto:
    def __init__(self):
        ...
        self.Gen_vxy_flat = self.parse_axis(('Gen_vxy_flat',800,-5,5)) 
        self.GED_vxy_flat = self.parse_axis(('GED_vxy_flat',800,-5,5)) 
        self.Lpt_vxy_flat = self.parse_axis(('Lpt_vxy_flat',800,-5,5)) 
...
def fillHistos(events,h,samp,cut,info,sum_wgt=1):
    ...
    if info["type"] == "signal":
        ...
        Gen_vxy_flat = ak.flatten(ak.concatenate(    [ev.GenEle.vxy[:, None], ev.GenPos.vxy[:, None]], axis=1))  #Genvxy
