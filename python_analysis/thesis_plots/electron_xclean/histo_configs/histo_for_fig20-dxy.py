...
def make_histograms():
    ...
    h.make('res_GED_gen_ptbin', 'Gen_pt_res', 'Res_GED')
    h.make('res_LPT_gen_ptbin', 'Gen_pt_res', 'Res_LPT')
...
def fillHistos(events,h,samp,cut,info,sum_wgt=1):
    ...
    if info["type"] == "signal":
        ...
        Lpt_pt_flat = ak.flatten(ak.concatenate([    lpt[ak.singletons(ev.GenEle.matchIdxAllLowPt)],    lpt[ak.singletons(ev.GenPos.matchIdxAllLowPt)]], axis=1))
        GED_pt_flat = ak.flatten(ak.concatenate([    ged[ak.singletons(ev.GenEle.matchIdxLocal)],    ged[ak.singletons(ev.GenPos.matchIdxLocal)]], axis=1))
        ...
        h.fill("res_GED_gen_ptbin", Gen_pt_res=Gen_dxy_FLAT, Res_GED=res_GED) #changed to dxy IMP

        h.fill("res_LPT_gen_ptbin",    Gen_pt_res=Gen_dxy_FLAT,    Res_LPT=res_LPT) #changed to dxy IMP
