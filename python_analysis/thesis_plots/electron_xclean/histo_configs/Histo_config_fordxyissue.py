...
class myHisto:
    def __init__(self):
        ...
        self.DXY = self.parse_axis(('DXY',500,-3,3))  
        
        self.dxy_gen = self.parse_axis(('dxy_gen',500,-3,3))  #just added
        ...
        self.GED_dxy_flat = self.parse_axis(('GED_dxy_flat',500,-3,3)) 
        self.Lpt_dxy_flat = self.parse_axis(('Lpt_dxy_flat',500,-3,3)) 
...
def make_histograms():
    ...
    h.make('Lpt_dxy_flat', 'Lpt_dxy_flat')
...
def fillHistos(events,h,samp,cut,info,sum_wgt=1):
    ...
    if info["type"] == "signal":
        ...
        #Resolution STUDY:
        DXY = ak.flatten(events.Electron.dxy)  #Trying to check dxy before selecting the events
        print ("events.AllLptElectron.dxy=", ak.flatten(events.AllLptElectron.dxy))
        ...
        print("GEN      GED       LPT       -GEN")

        # for g, ged, lpt in zip(    Gen_dxy_FLAT,    GED_dxy_flat,    Lpt_dxy_flat):
        #     print(g, ged, lpt, -g)
        ...
        h.fill("GED_dxy_flat", GED_dxy_flat=GED_dxy_flat)
        
        h.fill("Lpt_dxy_flat", Lpt_dxy_flat=Lpt_dxy_flat)
