...
class myHisto:
    def __init__(self):
        ...
        self.Gen_lxy_res = self.parse_axis(('Gen_lxy_res',[0,0.2,0.4,0.6,0.8,1,1.2,1.4,2])) #Gen_pt_FLAT in this case
...
def fillHistos(events,h,samp,cut,info,sum_wgt=1):
    ...
    if info["type"] == "signal":
        ...
        h.fill("res_GED_gen_ptbin", Gen_lxy_res=Gen_dxy_FLAT, Res_GED=res_GED) #changed to dxy IMP

        h.fill("res_LPT_gen_ptbin",    Gen_lxy_res=Gen_dxy_FLAT,    Res_LPT=res_LPT) #changed to dxy IMP
