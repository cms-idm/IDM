...
class myHisto:
    def __init__(self):
        ...
        self.Gen_dxy_res = self.parse_axis(('Gen_dxy_res',[0,1,3,6,15,30])) #Gen_lxy_FLAT in this case

        self.Gen_vxy = self.parse_axis(('Gen_vxy',[0,1,3,6,15,30])) 
...
def fillHistos(events,h,samp,cut,info,sum_wgt=1):
    ...
    if info["type"] == "signal":
        ...
        h.fill("res_GED_gen_ptbin", Gen_dxy_res=Gen_vxy_FLAT, Res_GED=res_GED) #changed to dxy IMP

        h.fill("res_LPT_gen_ptbin",    Gen_dxy_res=Gen_vxy_FLAT,    Res_LPT=res_LPT) #changed to dxy IMP
