...
def fillHistos(events,h,samp,cut,info,sum_wgt=1):
    ...
    if info["type"] == "signal":
        ...
        print ("Confirm=", events_with_genmatchedtoLPT.GenEle.matchType) #Confirmed

        print ("events_with_genmatchedtoLPT.AllLowPtElectron.pt=", events_with_genmatchedtoLPT.AllLptElectron.pt) #Checking the pT of alllpt of these events
        print ("events_with_genmatchedtoLPT.LptElectron.pt=", events_with_genmatchedtoLPT.LptElectron.pt)
        
        mask_LptandGED = events_with_genmatchedtoLPT.AllLptElectron.minDRtoReg < 0.05
        Imp_events = events_with_genmatchedtoLPT[mask_LptandGED]
        drcalcu = Imp_events.AllLptElectron.minDRtoReg 
        print ("drcalcu=", drcalcu)  #Confirming the dR
        
        lpt_being_lptandGED = Imp_events.AllLptElectron.pt #pT of lpt electrons (which are also GED)
