...
def fillHistos(events,h,samp,cut,info,sum_wgt=1):
    ...
    if info["type"] == "signal":
        ...
        h.fill("res_GED_gen_ptbin", Gen_pt_res=Gen_vxy_FLAT, Res_GED=res_GED) #changed to dxy IMP

        h.fill("res_LPT_gen_ptbin",    Gen_pt_res=Gen_vxy_FLAT,    Res_LPT=res_LPT) #changed to dxy IMP
