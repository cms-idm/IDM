...
def fillHistos(events, hists, samp, cut, info, sum_wgt=1):
    ...
    # select only events with at least one gen-matched vertex
    matched_vtx = events.vtx[events.vtx.isMatched]
    has_vtx = ak.num(matched_vtx) > 0
    ev  = events[has_vtx]
    vtx = ak.flatten(matched_vtx[has_vtx])
    wgt = ev.eventWgt / sum_wgt
    # broadcast weight to one entry per matched vertex
    wgt_vtx = ak.flatten(ak.broadcast_arrays(wgt[:, np.newaxis], matched_vtx[has_vtx])[0])
